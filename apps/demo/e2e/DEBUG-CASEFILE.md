...
- Loader regression from streaming/unlink changes (**control**: harness
  ...
  stage-runtime dist. Its parity page runs THREE wasm stage instances
...
`vite preview` (production bundle). This is the variable NEVER exercised
by any legion-stage-runtime harness spec (harness/playwright.config.ts
runs `npx vite --port 4173`, the DEV server, for every single spec
including the "proven green" `p2p.spec` control and the 3-instance
parity page). This asymmetry (control = dev server, mainline = prod
build) was not previously called out as ruled out and is a real gap in
the control.
...
mainline spec's exact shape concurrently" as the decisive test, the next
run switched to SEQUENTIAL loads (worker1 fully completes, `await`s,
THEN worker2 starts) to match the real driver-then-remote order
...
same-origin large-file fetches. It is parked, separate from the
mainline defect (mainline dies silently ~320 MB in, with no ErrorEvent
at all, and the real driver + remote loads are sequential, per
...
`BrowserContext` (unlike test 1's single-page, two-worker shape; that
shape IS provably one renderer). This test therefore doesn't cleanly
isolate "renderer process count" the way test 1 did, but it does rule
out "BrowserContext-per-peer" as the fix people assumed it was.
...
distinguishing factor is NOT page/context topology at all. Something
about the REAL page (mesh join, WebRTC/Trystero connections to 3 public
MQTT relay brokers, React app overhead, `useStageHost`'s
capability-detection/publish cycle) that debug-two-workers.html doesn't
have is the likely cause. The host in the real pipeline sits alive for
tens of seconds (mesh discovery, WebGPU limit detection, cap
publish/heartbeat) BEFORE `stage.load` ever arrives. debug-two-workers.html
starts the load immediately on page load. That points straight at
casefile hypothesis 3 (timing/aging).
...
**Pure wall-clock idle time is RULED OUT** as sufficient on its own: a
genuinely idle host (nothing touching the GPU, no mesh capability
...
(`packages/mesh-react/src/webgpuLimits.ts`) on mount. That call makes
...
already ruled out earlier via `?nochat=1`; every helper here already
passes that flag. It's controlled for.) Next: hypothesis 4, the natural
next candidate from combining test 1+2+3 evidence. Does merely
requesting a `GPUAdapter` from the main thread, before a worker in the
SAME renderer requests its own adapter+device, corrupt or destabilize
the worker's later WebGPU state? Testing directly in the harness (add
the exact `requestAdapter()` call demo's host makes, nothing else, to
the otherwise-untouched harness host) will isolate this one variable
cleanly.

## Test 4 (2026-07-15)

Added `legion-stage-runtime/harness/e2e/p2p-adapter-probe-host.spec.ts`:
same shape as test 3, except the host page runs
...
and never destroyed, immediately before the driver proceeds with its
normal `stage.load`.
...
**re-capped**". Every peer in the room re-broadcasts its cap on a
...
presence cap). `StagePipelinePanel` therefore re-renders roughly every
15s purely from roster churn, REGARDLESS of anything the local peer
itself is doing. Every such re-render created a new `log` closure. That
new closure changed the effect's dependency array. The changed array
tore the effect down.
...
the worker isn't yet holding a `stage` object mid-load and responds
...
it just silently stops the thread.** This is EXACTLY the casefile's
...
it. The 240s `loadDeadlineMs` `Promise.race` timeout eventually fires
and logs "stage worker load exceeded 240000ms (worker died silently or
stalled)". This message matches driver-side logs seen in every failing
run.

The consistent ~320 MB death point across 14+ runs makes sense under
this mechanism: a heartbeat-driven timer firing at a roughly fixed
wall-clock offset lines up with a roughly fixed number of streamed MB on
this box's local (loopback, disk-cached) `vite preview` static server.
This is timing-driven, and it exactly matches casefile hypothesis 3's
spirit, even though the precise mechanism (React effect teardown)
wasn't one of the 3 originally ranked hypotheses. It only became
visible after tests 1-4 eliminated every other axis and left "the real
`useStageHost` hook, and nothing else" as the one common factor between
every failing configuration.
...
(`logRef`, same pattern the file already uses for `baseCapRef`),
removing the direct dependency on `log`'s identity. The "answer" effect
now shadows `log` with a stable wrapper that reads `logRef.current`, and
`log` is removed from that effect's dependency array (with an explicit
comment documenting the deliberate exclusion, made because this
identity must never affect the effect's lifecycle). This is a
framework-level fix: it protects EVERY caller from an unstable logger
silently killing an in-flight worker load.
...
symlinks to `packages/mesh-react`'s rebuilt `dist/`, and the workspace
build was picked up automatically. No robocopy was needed for this fix,
