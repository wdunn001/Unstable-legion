...
still occupied pipeline slots for however long it lasted, and it's that
resource use being debited, not a reward being withheld.
...
point. The other half — "the scheduler reserves one optimistic slot per
round for the least-history peer" — is `StandingLedger.pickOptimisticSlot
(candidatePeerIds)`: it returns whichever candidate this ledger has the
fewest recorded events for (an unseen candidate always wins; ties broken
by earliest first-seen, then peerId for determinism), independent of
`priorityScore`. A scheduler (M2/M3, not built here) calls this once per
admission round alongside the normal priority pop to guarantee a
...
1-second time bucket, *not* `Math.random()`, so the same call is
reproducible in tests and so a peer can't just average the noise away by
polling in a tight loop within one scheduling round. The amplitude is
small enough (`0.25` against a `1.0`-wide newcomer-floor/lowest-lane gap)
...
**Sybil residual — honest, not hidden.** This ledger is deliberately
...
newcomer's score, not better. That's the intended containment, not a gap:
resetting sheds debt, but nets you back to the bottom of the "established
...
a residual, accepted risk, not a solved problem — flagging it here rather
