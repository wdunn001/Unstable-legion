...
or any browser without usable WebGPU can't do that. It can still
...
with no meaningful input activation. This is exactly what a capable driver's own
local stage 0 already does. The thin driver simply moves that first step
onto the mesh.

> **Privacy cost: stated plainly.** A capable driver computes the
> embedding + first layers locally. What leaves the device is already an
...
- **`planThinDriverRoute(topology, opts)`**: the thin analog of
  `planCommunalRoute`. It produces a `StagePlan` with **no synthetic local stage 0**:
  stage 1 is the remote isFirst host, and the last is isFinal.
- **`communalHostClaim(..., { firstLayer })`**: a host that opts in to
  supporting thin drivers passes `firstLayer: 0`; it then gap-fills from
  layer 0. Its lowest claim then owns the embeddings (`includeEmbeddings`): an
...
sees thin ads (its topology drops `layerStart < driverLayers`). Capable
...
A **mode flag on the same session state machine**. It is no forked function.
...
- **Single-hop is the wired case.** Like the capable path
  ...
  this repo yet. This is the identical documented limitation the capable path
  carries, already present before this feature.
...
things at different times. That is easy to conflate:
...
never uses. That is why stage 0 is loaded **lazily**, at this peer's first
chat (via `ensureResidentStageZero`), and never at hosting-enable time.
