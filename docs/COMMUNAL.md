...
with no coordinator, no election, no central planner**. That is the thing
that turns "everyone opens the app and it just becomes part of the mesh"
from an aspiration into an actual property of the mesh.
...
The coverage walk is a classic greedy "furthest reach wins" frontier
scan. Critically, every segment it produces carries a candidate's OWN
exact `(layerStart, layerEnd)`, never a trimmed sub-range. A driver can
...
headroom → priority → stability → RTT. It then applies an
**anti-stampede spread** (`hash(driverPeerId) % min(candidates,
spreadWidth=3)`) that spreads different drivers across the top few
candidates. This avoids a stampede onto the single globally-best host.
It returns a **normal `StagePlan`** (the exact same shape
`stagePlanner.ts#planPipeline` produces). Every existing plan-driven
UI/consumer therefore works unmodified. `communalAttachOrder` exposes
the same ranking as a full ordered candidate list (chosen-first, then
busy-fallback spares) for the driver's attach loop.
...
4. **Adjacent-gap growth**: an essential host with unused capacity
   ...
   in the same tick). This lets a host with slack absorb a dead
   neighbor's range without a dedicated pre-positioned spare.
5. **Wasteful-overlap yield**: a duplicate segment while a gap exists
   ...
   mesh-wide per tick. No two hosts both stay (thrash) or both leave
   (re-opens the gap).
...
idle hosts genuinely go idle). **Two real bugs were found and fixed by
these tests**: the thundering-herd stampede (essential-claim protection,
above) and a "tight tiling has no self-heal path" gap (adjacent-gap
growth, above). Both were invisible to single-decision unit tests. Both
only surfaced once the simulation ran enough random multi-agent rounds to
hit the adversarial timing.
...
`replanRoute` on failure (the legacy `runDriverStageSession` just
aborts). A communal route's built-in redundancy (warm spares) makes a
second attempt cheap and often successful even when planning was already
slightly stale.
...
gap, and additional hosts become warm spares of that SAME segment. The
attach-order fallback already handles this case.
...
- **Immediate republish**: every `syncPublicState()` call (session
  open/free/queue change) now also triggers a same-tick republish,
  tightening the update cadence from the 15s heartbeat baseline. A
  driver deciding "does this host have a free lane right now" needs
  current occupancy.
- New `preloadStage` option: proactively load a stage BEFORE any driver
  ever sends `stage.load`/`stage.session.open`. This reuses the existing
  `ensureWorkerLoaded` reuse/reload/conflict rules verbatim. That is
...
- New `suppressAdvertise` option: publish `stageHost` WITHOUT
  ...
  serving. This is the "stop advertising" half of graceful teardown.
- New `useMemoryShardStore` plumbing (`StageWorkerClient.load`,
  `stageWorkerProtocol.ts`'s `StageWorkerRequest`, `apps/demo`'s
  `stageWorker.ts`): forces the in-memory `ShardStore` for a load,
  bypassing OPFS. Additive and off by default.
...
- `useStageHost` only ever populates/publishes `loadedStages` AFTER
  `ensureWorkerLoaded` resolves (which is after `warmUpStageWorker`).
  "Never advertise a not-warm stage" (the C3 cold-shader lesson) is
  therefore structural here.
...
disconnect after grace is follow-up work for a later pass.
...
- **The M3 e2e acceptance test now exists and is green**
  ...
  covers the whole communal range solo. The OTHER two hosts therefore
  become warm spares of the exact same segment. (See
  `stageOrchestrator.ts`'s SCOPE NOTE on why this is the common,
  ...
  both. Detection latency is ~1.6-2.1s, non-lockstep, with 0-510ms
  stagger observed across runs. BOTH finish with `restartCount:1`,
...
- **No manifest exists yet for `qwen3-0.6b-q8_0`** (or any model) at this
  ...
  manifest for this model is separate follow-up work. The communal
  pipeline functions correctly today via the fallback path (exercised,
  proven, and what's actually deployed).
- **Multi-hop relay across >1 remote communal segment** still has no
  implementation (a limitation carried over from the legacy
  `runDriverStageSession`). Not exercised by `communal.spec.ts` either.
  The real assembled topology in this pass's runs always converged to
  exactly one segment (see above). That is the scope both driver session
  functions actually support end to end.

## Test matrix

`packages/mesh-core`: 164/164 (`npm test`). Breakdown: 119 + 20
...
`packages/mesh-react`: 38/38 (`npm test`). Breakdown: 34 pre-existing + 4
new `economyWiring.test.ts` (telemetry -> `StandingLedger` -> priority,
...
jsdom/testing-library harness to render the hooks themselves. The pure
logic is pulled out and tested directly, following the same precedent as
...
while doing this. They surfaced only once a SECOND panel with an
overlapping class name (`CommunalChatPanel`'s `.sp-prompt`) was added to
the same page: `StagePipelinePanel`'s host toggle and prompt input gained
disambiguating classes (`stage-host-toggle`, `stage-pipeline-prompt`)
since `.sp-host-toggle`/`.sp-prompt` alone now match more than one
element once `CommunalHostPanel` (added by the ORIGINAL M3 pass) and
`CommunalChatPanel` (this pass) are both always rendered in `Dashboard`.
Every e2e call site
