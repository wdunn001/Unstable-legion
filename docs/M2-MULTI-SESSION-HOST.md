...
several concurrent driver sessions, without tearing itself down on a
second `stage.load`.

## sessionId routing: the mesh-core envelope, with no Codec frame field involved
...
`frames.ts`). That file stays untouched and doesn't need to know sessions
...
after open is the wire header" convention. That convention only worked for exactly
...
- **legacy** (`stage.load`, what `stageOrchestrator.ts`'s driver still
  ...
  per-session, no longer globally. Replies `stage.ready` (unchanged wire
...
SAME in-flight promise, without racing a second `.load()`. This was caught
during e2e development. Unit tests never caught it (this is exactly the kind of
...
IDENTICAL at `max_sessions=1` vs `4` in the reference build. That
formula is therefore a conservative planning upper bound. It carries no real
per-session cost.
...
above therefore exercises only the LEGACY origin path (still genuinely
concurrent, still sessionId-enveloped over `sf`, still isolated). It does
not yet exercise the new kind. Wiring a driver onto `stage.session.open` (to get
`stage.session.busy`/queueing exercised live, beyond just the pure
admission unit tests) is follow-up work, still to come.
...
`await`s (WebGPU dispatch + readback are real async yield points, with no
synchronous-stub shortcut) into the SAME wasm instance / GPU device. This is a
DIFFERENT concurrency shape than legion-stage-runtime's M1 harness gate.
That harness alternates "session A one full step, THEN session B one full
step": it never lets two steps' async work genuinely overlap.

Fixed by serializing every native/wasm dispatch through one promise chain
in the worker (`serialize()` wrapping `handle()`). Only one call is
...
minutes (the idle-eviction window) after a successful run. This was caught by
...
`stage.stop` on every natural finish too (`notifyRemotesDone`), beyond just
