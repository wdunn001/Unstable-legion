...
work — the gap was entirely in wiring, not codec logic:
...
- No activation ever flows host → driver: the only reverse-direction
  ...
  message (`tc` action), not an `sf` activation frame. Per
  ...
  this repo today — there is no host-to-host relay loop, so there is no
...
- `apps/chat/src/App.tsx`: both hook calls now pass
  `modelConfig.wireDtype` instead of the hardcoded literal.
- `App.tsx`'s `__legionChat` debug snapshot now also exposes
  `chatTokens` (`chat.tokens`, the raw generated token-id sequence) — the
  exactness e2e needs the exact greedy-decode token stream, not just its
  detokenized text.
...
- `packages/mesh-core/test/stageOrchestrator.test.ts` (extended) — the
  mock host gained an opt-in `decodeWire: true` mode that does what a real
  `useStageHost.ts` does with inbound `sf` bytes (unwrap envelope, build a
  real `ActivationWireDecoder`, `decodeFrameBytes` every frame) instead of
  ignoring them. Two new tests: an f16 route delivers f32 buffers of the
...
- `apps/chat/e2e/wire-dtype.spec.ts` (new, `chromium-webgpu` project) — a
  **solo self-hosting tab** (`?testModel=1&wireDtype=<f32|f16>`, this same
  ...
  hard-fail on divergence; per-prompt exactness is a report, not a gate.
...
generations (47+ and 162+ tokens) diverged partway through — expected for
...
diverges (the KV-cache/generation state on each side is now genuinely
...
tests) — but greedy decode is a knife-edge sampler, and this run directly
...
UX/trust cost, not just a numerics footnote. Suggested rollout: ship
...
See the section below once Phase 2 lands; this file is appended to, not
rewritten, so Phase 1's mechanism/results stay intact as a reference for
