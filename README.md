...
parallelism over WebRTC). No single browser has to hold all 8B parameters.
...
- **Communal hosts** each claim a layer range via a self-assembly loop
  ...
  (`docs/COMMUNAL.md`).
- Only the boundary **activations** cross the wire (f16 by default; see
...
- **Signaling** (WebRTC offer/answer exchange, peer discovery): an
  **MQTT-over-WSS** broker, configured via `VITE_RELAY_URLS`. Run your own.
  ...
  SDP. This is a silent failure: without a data channel, a peer never enters
  the roster at all. It simply looks like "nobody is home."
...
(`legion-stage.{js,wasm}`). This is **not** published or checked in. Build it
...
- `VITE_TURN_URLS` / `VITE_TURN_USERNAME` / `VITE_TURN_CREDENTIAL`: your TURN
  relay. **A missing `.env` silently ships a STUN-only bundle**. That strands
