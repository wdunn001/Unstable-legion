...
peers over the existing `tc` tool-RPC bus — and its reverse-direction
twin, **text → Kokoro TTS in a Web Worker → audio clip**, exposed the
...
TTS — the reverse-direction twin, same shapes, same idioms:
...
- `kokoroEngine.ts` — the only TTS engine implemented so far:
  ...
  (`dtype: 'q8'`) — kokoro-js's own recommended pairing per device.
...
## Why Whisper/Kokoro via transformers.js, not Parakeet/Piper
...
in the lockfile, so TTS reuses it too — still only one onnxruntime-web
copy across both capabilities. Parakeet.js (streaming/low-latency ASR)
...
output clip — simple, and it reuses the exact tool-RPC bus every other
mesh tool already speaks. The cost is ~33% base64 overhead and no
streaming (a whole clip travels in one JSON message, so long clips will
...
the wire — not implemented here; this PoC intentionally keeps the
...
- `kokoro-*.js` — **~1.33 MB** minified (kokoro-js + its `phonemizer`
  ...
  switched on — a separate chunk from ASR's `transformers.web-*.js`, so
...
- A second `transformers.web-*.js` chunk (**~869 KB**, same size as
  ...
  into its own chunk rather than merging with Whisper's; the underlying
...
mirror the same way `mirror-whisper-to-cdn.sh` does; not written yet.
